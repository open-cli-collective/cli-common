...
Four family-wide flag names form a coordinate system for "how should the result render?" — orthogonal to *what* is fetched. **Family-wide** means the names and semantics are reserved across all CLIs; registration is per-command (NOT a cobra `PersistentFlag` on root), so each command exposes only the flags meaningful to its output. `--id` MUST be registered on every list/get command. `--extended`/`--fulltext`/`--fields` MUST be registered when there is corresponding surface (admin/schema columns, prose cells, per-row columns) and MUST NOT be registered when they would be a no-op — `--help` should accurately advertise what the command supports. All four are long-only (cross-ref `command-surface.md` §7.2).
