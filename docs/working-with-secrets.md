...
- **Explicit user opt-in to the `pass` backend** is supported via the same knobs (`--backend pass`, `<SERVICE>_KEYRING_BACKEND=pass`, or `keyring.backend: pass`). `pass` is the canonical headless-Linux answer for users who already run a `pass` password-store; it requires the `pass` CLI binary on `$PATH` and an initialized password-store (`pass init <gpg-key-id>`). cli-common scopes each consumer's entries under a per-service subtree of the password-store (the credstore sets `PassPrefix = <service>`), so two CLIs storing the same key name do not collide. KeePassXC users do not need a separate backend — KeePassXC integrates with Secret Service over D-Bus, so `--backend secret-service` (the default) already covers it. The `pass` backend is `!windows`: requesting it on Windows fails closed with a "no such backend" error rather than silently falling back.
